        ...
        if (command.type == LiteScript.CommandType.Dialogue) {
            ...
            if (commandContent.effect) {
                if (pendingShowImages.length > 0) {
                    showAllPendingImages(pendingShowImages, localFile);
                    pendingShowImages = [];
                }
            ...
            if (commandContent.name == '表示') {
                ...
                if (ayumiGlobalMapInfo.includes(block.id) && (commandContent.name == '背景' || commandContent.name == '注意' || commandContent.name == '表示２' || commandContent.name == '天気') || block.name.startsWith('Target') || block.name.startsWith('Conversation') || block.name.startsWith('Character')) {
                    ...
                    if (commandContent.effect && commandContent.effect.extraData.type == LiteScript.EffectType.None) {
                        ...
                        if (pendingShowImages.length > 0) {
                            pendingShowImages.push({
                                name: name.id,
                                target: commandContent.source,
                                transform: null,
                                zorder: null
                            });
                            showAllPendingImages(pendingShowImages, localFile);
...
function showAllPendingImages(list: Renpy.PendingImage[], localFile: RenpyFile,): void {
    let group = list.map(image => ({
